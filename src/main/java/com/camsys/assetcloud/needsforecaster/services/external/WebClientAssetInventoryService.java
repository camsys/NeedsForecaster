...
public class WebClientAssetInventoryService implements AssetInventoryService {
    ...
    @Override
    public void broadcastAssetUpdates(String token, List<Asset> assets) throws Exception {
        // Get org name from the first asset in the list
        List<Org> allOrgsForUser = this.getOrgs(token);
        if (assets.size() != 0) {
            Optional<Org> optionalOrg = allOrgsForUser.stream().filter((o) -> {return Objects.equals(o.orgKey, assets.get(0).orgKey);}).findFirst();
            if (optionalOrg.isPresent()) {
                String orgName = optionalOrg.get().toString();
                ...
                HttpEntity<?> entity = createHttpEntity(requestBody, token);
            ...
            System.out.println("No active assets.");
        }
    }

    private HttpEntity<?> createHttpEntity(Object body, String token) {
    ...
    private HttpEntity<?> createHttpEntity(String token) {
        return this.createHttpEntity(null, token);
    }
